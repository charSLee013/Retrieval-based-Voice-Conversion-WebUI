...
def vc_multi(
    ...
    print(f"opt_root: {opt_root}")  # opt
    print(f"paths: {paths}")    # tempfile._TemporaryFileWrapper
    print(f"f0_up_key: {f0_up_key}")    # 0.0
    print(f"f0_method: {f0_method}")    # rmvpe
    print(f"file_index: {file_index}")  # ""
    print(f"file_index2: {file_index2}") # ""
    print(f"index_rate: {index_rate}")  # 1
    print(f"filter_radius: {filter_radius}")    # 3
    print(f"resample_sr: {resample_sr}") # 0
    print(f"rms_mix_rate: {rms_mix_rate}") # 1
    print(f"protect: {protect}") # 0.33
    print(f"format1: {format1}") # wav
    try:
        dir_path = (
            ...
            with gr.Group():  # 暂时单人的, 后面支持最多4人的#数据处理
                ...
                with gr.Row():
                    trainset_dir4 = gr.Textbox(
                        label=i18n("输入训练文件夹路径"), value="/root/"
                    )
                    spk_id5 = gr.Slider(
                        minimum=0,
                        maximum=4,
